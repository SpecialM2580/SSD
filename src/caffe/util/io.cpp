...
bool ReadTxtToAnnotatedDatum(const string& labelfile, const int height,
  ...
  while (getline(infile,iminfo)) {
    elem=split(iminfo,";");
    xmin=atof(elem[1].c_str());
    ymin=atof(elem[2].c_str());
    xmax=atof(elem[3].c_str());
    ymax=atof(elem[4].c_str());
    label=1;
    LOG(INFO)<<"name:"<<elem[0];
    Annotation* anno = NULL;
    int instance_id = 0;
    bool found_group = false;
    LOG(INFO)<<"anno_datum:"<<anno_datum->annotation_group_size();
    for (int g = 0; g < anno_datum->annotation_group_size(); ++g) {
      AnnotationGroup* anno_group = anno_datum->mutable_annotation_group(g);
      LOG(INFO)<<"label:"<<label<<";anno_group:"<<anno_group->group_label();
      if (label == anno_group->group_label()) {
        if (anno_group->annotation_size() == 0) {
          instance_id = 0;
        } else {
